...
export function setupBoggleHandlers(
  ...
  socket.on("submit-word", ({ word, path, rotationVersion }) => {
    ...
    debugLog(
      "EVENT: submit-word",
      { word, pathLength: path?.length, clientRotationVersion: rotationVersion },
      socket.id
    );
  ...
  socket.on("rotate-board", () => {
    ...
    if (result.success) {
      ...
      socket.emit("rotation-error", { message: result.reason! });
    }
  });
  ...
  socket.on("get-max-score", () => {
    ...
    if (
      ...
      debugLog(
        "EMIT: max-score-data",
        { totalWords: maxScoreData.totalWords, maxScore: maxScoreData.maxScore },
        socket.id
      );
      socket.emit("max-score-data", maxScoreData);
    } else {
      debugLog("EMIT: max-score-data (empty - no valid board)", null, socket.id);
      socket.emit("max-score-data", { words: [], maxScore: 0, totalWords: 0 });
    }
  });
